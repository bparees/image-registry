...
type Interface interface {
	Import(ctx gocontext.Context, isi *imageapiv1.ImageStreamImport, stream *imageapiv1.ImageStream) error
}
...
type ImageStreamImporter struct {
	...
	digestToRepositoryCache map[gocontext.Context]map[manifestKey]*imageapiv1.Image
...
func NewImageStreamImporter(retriever RepositoryRetriever, maximumTagsPerRepo int, limiter flowcontrol.RateLimiter, cache *ImageStreamLayerCache) *ImageStreamImporter {
	...
	return &ImageStreamImporter{
		...
		digestToRepositoryCache: make(map[gocontext.Context]map[manifestKey]*imageapiv1.Image),
		digestToLayerSizeCache:  cache,
	}
}

// Import tries to complete the provided isi object with images loaded from remote registries.
func (i *ImageStreamImporter) Import(ctx gocontext.Context, isi *imageapiv1.ImageStreamImport, stream *imageapiv1.ImageStream) error {
	...
	if _, ok := i.digestToRepositoryCache[ctx]; !ok {
		i.digestToRepositoryCache[ctx] = make(map[manifestKey]*imageapiv1.Image)
	}
...
func (i *ImageStreamImporter) importImages(ctx gocontext.Context, retriever RepositoryRetriever, isi *imageapiv1.ImageStreamImport, stream *imageapiv1.ImageStream, limiter flowcontrol.RateLimiter) {
	...
	isi.Status.Images = make([]imageapiv1.ImageImportStatus, len(isi.Spec.Images))
	for i := range isi.Spec.Images {
		...
		if len(defaultRef.ID) > 0 {
			...
			preferArch := tagReference.Annotations[imageapiv1.ImporterPreferArchAnnotation]
			preferOS := tagReference.Annotations[imageapiv1.ImporterPreferOSAnnotation]
...
// status field is set.
func (i *ImageStreamImporter) importFromRepository(ctx gocontext.Context, retriever RepositoryRetriever, isi *imageapiv1.ImageStreamImport, maximumTags int, limiter flowcontrol.RateLimiter) {
	if isi.Spec.Repository == nil {
		return
	}
	cache := i.digestToRepositoryCache[ctx]
	isi.Status.Repository = &imageapiv1.RepositoryImportStatus{}
	...
	status.Images = make([]imageapiv1.ImageImportStatus, len(repo.Tags))
...
func formatRepositoryError(ref imageapiv1.DockerImageReference, err error) error {
	switch {
	case isDockerError(err, v2.ErrorCodeManifestUnknown):
		err = kapierrors.NewNotFound(imageapiv1.Resource("dockerimage"), ref.Exact())
...
func (isi *ImageStreamImporter) calculateImageSize(ctx gocontext.Context, bs distribution.BlobStore, image *imageapiv1.Image) error {
...
func manifestFromManifestList(ctx gocontext.Context, manifestList *manifestlist.DeserializedManifestList, ref imageapiv1.DockerImageReference, s distribution.ManifestService, preferArch, preferOS string) (distribution.Manifest, error) {
...
func (isi *ImageStreamImporter) importManifest(ctx gocontext.Context, manifest distribution.Manifest, ref imageapiv1.DockerImageReference, d godigest.Digest, s distribution.ManifestService, b distribution.BlobStore, preferArch, preferOS string) (image *imageapiv1.Image, err error) {
...
func (isi *ImageStreamImporter) importRepositoryFromDocker(ctx gocontext.Context, retriever RepositoryRetriever, repository *importRepository, limiter flowcontrol.RateLimiter) {
	...
	if err != nil {
		...
		case isDockerError(err, v2.ErrorCodeNameUnknown):
			err = kapierrors.NewNotFound(imageapiv1.Resource("dockerimage"), repository.Ref.Exact())
		case isDockerError(err, errcode.ErrorCodeUnauthorized):
...
func importRepositoryFromDockerV1(ctx gocontext.Context, repository *importRepository, limiter flowcontrol.RateLimiter) {
	...
	if value == nil {
		err := kapierrors.NewForbidden(imageapiv1.Resource(""), "", fmt.Errorf("registry %q does not support the v2 Registry API", repository.Registry.Host))
...
// manifestKey is a key for a map between a Docker image tag or image ID and a retrieved imageapiv1.Image, used
...
func imageImportStatus(err error, kind, position string) metav1.Status {
	...
	case kapierrors.APIStatus:
		return t.Status()
	case *field.Error:
		return kapierrors.NewInvalid(imageapiv1.Kind(kind), position, field.ErrorList{t}).ErrStatus
	default:
		return kapierrors.NewInternalError(err).ErrStatus
	}
}

func setImageImportStatus(images *imageapiv1.ImageStreamImport, i int, tag string, err error) {
...
func invalidStatus(position string, errs ...*field.Error) metav1.Status {
	return kapierrors.NewInvalid(imageapiv1.LegacyKind(""), position, errs).ErrStatus
}
