...
func init() {
	err := kapi.Scheme.AddConversionFuncs(
		...
		func(in *docker.Image, out *imageapiv1.DockerImage, s conversion.Scope) error {
		...
		func(in *imageapiv1.DockerImage, out *docker.Image, s conversion.Scope) error {
			if err := s.Convert(&in.Config, &out.Config, conversion.AllowDifferentFieldTypeNames); err != nil {
				return err
			}
			if err := s.Convert(&in.ContainerConfig, &out.ContainerConfig, conversion.AllowDifferentFieldTypeNames); err != nil {
...
func normalizeDockerHubHost(host string, v2 bool) string {
	switch host {
	case imageapiv1.DockerDefaultRegistry, "www." + imageapiv1.DockerDefaultRegistry, imageapiv1.DockerDefaultV1Registry, imageapiv1.DockerDefaultV2Registry:
		if v2 {
			return imageapiv1.DockerDefaultV2Registry
		}
		return imageapiv1.DockerDefaultV1Registry
	}
	return host
}
...
func normalizeRegistryName(name string) (*url.URL, error) {
	prefix := name
	if len(prefix) == 0 {
		prefix = imageapiv1.DockerDefaultV1Registry
	}
	hadPrefix := false
	switch {
...
func (c *connection) ImageTags(namespace, name string) (map[string]string, error) {
	if len(namespace) == 0 && imageapiv1.IsRegistryDockerHub(c.url.Host) {
		namespace = imageapiv1.DockerDefaultNamespace
	}
	if len(name) == 0 {
...
func (c *connection) ImageByTag(namespace, name, tag string) (*Image, error) {
	...
	searchTag := tag
	if len(searchTag) == 0 {
		searchTag = imageapiv1.DefaultImageTag
	}
