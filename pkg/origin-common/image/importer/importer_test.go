...
func TestImport(t *testing.T) {
	...
	testCases := []struct {
		...
		isi       imageapiv1.ImageStreamImport
		expect    func(*imageapiv1.ImageStreamImport, *testing.T)
	}{
		{
			...
			isi: imageapiv1.ImageStreamImport{
				Spec: imageapiv1.ImageStreamImportSpec{
					Images: []imageapiv1.ImageImportSpec{
						{From: kapi.ObjectReference{Kind: "DockerImage", Name: "test"}, ImportPolicy: imageapiv1.TagImportPolicy{Insecure: true}},
					},
				},
			},
			expect: func(isi *imageapiv1.ImageStreamImport, t *testing.T) {
		...
		},
		{
			...
			isi: imageapiv1.ImageStreamImport{
				Spec: imageapiv1.ImageStreamImportSpec{
					Repository: &imageapiv1.RepositoryImportSpec{
						From: kapi.ObjectReference{Kind: "DockerImage", Name: "test"},
					},
				},
			},
	...
	for i, test := range testCases {
		...
		if err := im.Import(nil, &test.isi, &imageapiv1.ImageStream{}); err != nil {
			t.Errorf("%d: %v", i, err)
		}
		if test.expect != nil {
